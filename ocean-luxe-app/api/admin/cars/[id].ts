...
async function handler(req: ApiRequest, res: ApiResponse) {
  ...
  if (req.method === "GET") {
    const { rows } = await pool.query(`select * from car_types where id = $1 limit 1`, [id]);
    if (!rows[0]) {
  ...
  if (req.method === "PATCH") {
    ...
    const update = buildUpdatePatch(body, [
      ...
      "category",
      "seats",
      "range_estimate_miles",
      "luggage_capacity_notes",
      "is_active",
      "base_daily_rate",
      "default_markup_percent",
      "cleaning_fee",
      "delivery_fee_orlando",
    ]);
    if (!update) {
    ...
    const { rows } = await pool.query(
      `update car_types set ${update.sql}, updated_at = now() where id = $${update.values.length + 1} returning *`,
      [...update.values, id]
    );
    res.status(200).json(rows[0]);
    return;
