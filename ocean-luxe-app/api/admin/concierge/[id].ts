...
async function handler(req: ApiRequest, res: ApiResponse) {
  ...
  if (req.method === "GET") {
    const { rows } = await pool.query(`select * from concierge_services where id = $1 limit 1`, [id]);
    if (!rows[0]) {
  ...
  if (req.method === "PATCH") {
    ...
    const { rows } = await pool.query(
      `update concierge_services set ${update.sql}, updated_at = now() where id = $${update.values.length + 1} returning *`,
      [...update.values, id]
    );
    res.status(200).json(rows[0]);
    return;
