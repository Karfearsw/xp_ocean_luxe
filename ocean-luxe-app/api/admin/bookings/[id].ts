...
async function handler(req: ApiRequest, res: ApiResponse) {
  ...
  if (req.method === "PATCH") {
    ...
    const update = buildUpdatePatch(body, ["booking_status", "payment_status", "provider_confirmation_number", "notes_internal"]);
    if (!update) {
      res.status(400).json({ message: "No valid fields" });
      return;
    }
    const { rows } = await pool.query(
      `update bookings set ${update.sql}, updated_at = now() where id = $${update.values.length + 1} returning *`,
      [...update.values, id]
    );
    res.status(200).json(rows[0]);
    return;
